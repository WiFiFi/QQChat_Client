...
public class MainApplication extends Application {
    ...
    public void sendAction(String action, String otherId, String msgText) {
        ...
        // 拼接完整的聊天消息
        String content = String.format("%s,%s,%s,%s,%s,%s%s%s\r\n",
                action, androidId, getNickName(), DateUtil.getNowDateTime(""),
                otherId, "N", ClientThread.SPLIT_LINE, msgText);
    ...
    // 发送聊天消息(重载)
    public void sendAction(String action, String otherId, String msgText, String password) {
        String androidId = android.provider.Settings.Secure.getString(getApplicationContext().getContentResolver(), android.provider.Settings.Secure.ANDROID_ID);
        // 拼接完整的聊天消息
        String content = String.format("%s,%s,%s,%s,%s,%s%s%s\r\n",
                action, androidId, getNickName(), DateUtil.getNowDateTime(""),
                otherId, password, ClientThread.SPLIT_LINE, msgText);
        Log.d(TAG, "sendAction : " + content);
        // 获得一个默认的消息对象
        Message msg = Message.obtain();
        msg.obj = content; // 消息描述
